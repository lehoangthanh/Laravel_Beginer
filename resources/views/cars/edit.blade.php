...
<body>
    <div class="container py-4">
        <h2 class="text-danger">EDIT CAR</h2>
    </div>
    @if ($errors->any())
    ...
    @endif
    <form class="container" action="{{ url('/cars/update/'.$car->id) }}" method="post">
        {!! csrf_field() !!}
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="launched_year" value="{{ $car->launched_year }}">
            @error('launched_year')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
